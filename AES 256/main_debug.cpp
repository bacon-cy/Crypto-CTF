...
int main() {
    ...
    output.open("output.txt", std::ios::app | std::ios::binary);
    if(output.fail()){
        cerr<<"ERROR: Can not create output file.\n";
        exit(0);
    }
...
void keyGeneration(const byte* key){
    ...
    for(int i=8;i<60;i++){ //each words
        for(int _=0; _<4; _++) temp[_] = roundKey[i-1][_];
//        printf("%2d  ",i); for(int _=0; _<4; _++) printf("%02x",temp[_]); printf(" ");
        if(i % Nk == 0){
            rotWord(temp);
//            for(int _=0;_<4;_++) printf("%02x", temp[_]); printf("  ");
            subWord(temp);
//            for(int _=0;_<4;_++) printf("%02x", temp[_]); printf("  ");
            temp[0] ^= Rcon[i/Nk]; //for(int _=0; _<4; _++) temp[_]^= Rcon[i/Nk]; cool!
//          for(int _=0;_<4;_++) printf("%02x", temp[_]); printf("  ");
        }else if(i % Nk == 4){
            subWord(temp);
        }
        ...
        printf("temp  :");
        for(int _=0; _<4; _++) printf( " %2x", temp[_]);
        printf("\n");
    }
}
...
void shiftRows(byte* _state){
    ...
    for(int c=1;c<Nb;c++){ // c: column
        for(int k=0;k<Nb;k++) _temp[k] = _state[k*Nb + c]; // the first column
        for(int k=0;k<Nb;k++){
...
void decrypt(){
    //init round, key is Nr round key
